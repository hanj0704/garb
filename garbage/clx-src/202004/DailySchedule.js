...
//			subtractRunTime(vsBroadStart, vsBroadRun)
		...
		var vaAllRows = vcGrid.findAllRow("mtrlClf != 'SE'");
		var voSelectRow = vcGrid.getRow(vnRowIndex);
		var vaFilteredRow = vaAllRows.filter(function(each){
		
			if(each.getIndex() > vnRowIndex) {
				return each;
			}
		});
		var tempRow = voSelectRow;
		vaFilteredRow.forEach(function(each,idx){
...
function onBtnMtrlChangeClick( /* cpr.events.CMouseEvent */ e) {
    ...
    app.openDialog("app/operation/popup/MtrlChange", {
        width: 1000,
        height: 600
    }, function(dialog) {
        dialog.ready(function(dialogApp) {
            // dialogApp.initValue = voSelRow;
            dialog.headerTitle = "소재 변경";
        });
    }).then(function(/*Array*/returnData) {
        ...
        if (returnData || returnData.length > 0) {
			...
			returnData.forEach(function(each){
				...
				vaAllRows.forEach(function(eachRow, idx){
					...
					eachRow.setValue("mtrlId", returnData.mtrlIdAfter);
					eachRow.setValue("mtrlNm", returnData.mtrlNmAfter);
					eachRow.setValue("mtrlClf", returnData.mtrlClfAfter);
					eachRow.setValue("mtrlInfo", returnData.mtrlInfoAfter);
					eachRow.setValue("playRun", returnData.durationAfter);
					
				});
						
			});
			
			var vaProgramRows = vcGrid.findAllRow("mtrlClf != 'SE'");
			...
			vaProgramRows.forEach(function(each){
				...
				each.setValue("playTime24", vsPlayRun);
				tempRow = each;	
			});
