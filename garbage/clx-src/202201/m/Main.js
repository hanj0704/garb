...
exports.openMenuItemWithAppId = function (psAppId) {
	if (ValueUtil.isNull(psAppId)){
		return;
	}
	
	var vcMdiCn = app.lookup("mdiCn");
	var voAlreadyOpenedItem = vcMdiCn.findItemWithAppID(psAppId);
	...
	/* 화면분할 유지한 채 메뉴 오픈 */
	vcMdiCn.addItemWithApp(psAppId, true);
}
...
function setMenuFavorite(psAppId,pbFavorite){
	var vcDsFav = app.lookup("dsMenuFavorite");
	...
	if(pbFavorite) {
		...
		if(voRow) {
			...
			voRowData["FAVORITE_APP_ID"] = voRowData["APP_ID"];
			
			vcDsFav.addRowData(voRowData);
		}
	} else {
		var voRow = vcDsFav.findFirstRow("FAVORITE_APP_ID == '"+psAppId+"'")
		if(voRow) {
			
			var vnRowIndex = voRow.getIndex();
			vcDsFav.deleteRow(vnRowIndex);
		}
	}
...
function checkFavoriteMenu(){
	
	var vcDsMenu = app.lookup("dsMenu");
	var vcDsFav = app.lookup("dsMenuFavorite");
	
	var vaFavAppIds = vcDsFav.getColumnData("FAVORITE_APP_ID");
				
	vcDsMenu.forEachOfUnfilteredRows(function(dataRow){
		var vsAppId = dataRow.getValue("APP_ID");
		if(vaFavAppIds.indexOf(vsAppId) != -1) {
			
			dataRow.setValue("IS_FAVORITE", "true");
		} else {
			dataRow.setValue("IS_FAVORITE", "");
		}
	});
	
	util.Control.redraw(app,["treMenu2","treMenu3"]);
}

exports.checkFavoriteMenu = checkFavoriteMenu;
/**
...
function onBodyLoad(/* cpr.events.CEvent */ e){
	...
	util.Submit.send(app,"subMenuList",function(){
		...
		util.Submit.send(app, "subFavList", function(pbSuccess) {
			if (pbSuccess){
 ...
 * CheckBox의 value를 변경하여 변경된 값이 저장된 후에 발생하는 이벤트.
 */
function onCbxLoadTypeValueChange( /* cpr.events.CValueChangeEvent */ e) {
	/** 
	 * @type cpr.controls.CheckBox
	 */
	var cbxLoadType = e.control;
//	app.lookup("optLoadType").value = e.newValue;
	app.setAppProperty("loadType", e.newValue);
}
