...
function onBodyLoad(/* cpr.events.CEvent */ e){
	...
	if(vcCtrl){
		app.lookup("optTilte").value = vcCtrl.fieldLabel || app.getAppProperty("title");
		if(vcCtrl.type == "grid"){
			if(vcCtrl.dataSet){
...
//			app.lookup("grp1").getLayout().setColumnVisible(2, false);
			app.getContainer().getLayout().setColumnVisible(2, false);
		}
	}else if(e.property == "title"){
		app.lookup("optTilte").value = e.newValue;
		//kimjj 추가 
	}else if(e.property == "showGrdCount"){
		if( e.newValue == false ){
			app.lookup("grpGrdCnt").visible = false ;
		}
	}else if(e.property == "showCloseBtn" ){
		var voTempBtn = app.lookup("btnClose") ;
		
		if( voTempBtn == null || voTempBtn == undefined){
			
			var voTempBtn = new cpr.controls.Button("btnClose");
				voTempBtn.visible = false;
				voTempBtn.value = "";
				voTempBtn.icon = "theme/images/controls/button/ico_btn.png";
				voTempBtn.style.setClasses(["btn-transparent", "btn-i-group-close"]);
				if(typeof onBtnCloseClick == "function") {
					voTempBtn.addEventListener("click", onBtnCloseClick);
				} 
				
				app.lookup("grpTitle1").insertChild(0, voTempBtn, {
					"autoSize": "height",
					"width": "15px",
					"height": "100%"
				}); 
...
//	ctxMenu.addItem(new cpr.controls.TreeItem("컬럼숨김", "5", "root"));
	...
	ctxMenu.addEventListener("item-click", function(/**@type cpr.events.CItemEvent */ e){
		...
		// 그리드 컬럼 필터팝업 (2021.10.05 추가)
			...
			var vnLeft = parseInt(_.clone(showConstraint.left)) - (vnWidth/2);
...
function onBtnCloseClick(/* cpr.events.CMouseEvent */ e){
	...
	var voHosCtr = btnClose.getAppInstance().getHost();
    var voHostApp = btnClose.getAppInstance().getHostAppInstance();
    var voAppContainer = voHostApp.getContainer();

    

	//icon변경 
	var vsClass = btnClose.style.getClasses();
	
	for( var i=0 ; vsClass.length > i  ; i++ ){
		if( "btn-i-group-close" == vsClass[i] ){
			btnClose.style.addClass("btn-i-group-open");
			btnClose.style.removeClass("btn-i-group-close");
			break;
		} 
		
		if( "btn-i-group-open" == vsClass[i] ){
			btnClose.style.addClass("btn-i-group-close");
			btnClose.style.removeClass("btn-i-group-open");
			break;
		} 
	} 
	
	//user Event 
	var commonEvent = new cpr.events.CUIEvent("closeClick"); 
	var visCont = app.dispatchEvent(commonEvent);	 
	
	
//	if( visCont != null || visCont == false ) return ;  
//	var voCon1 = voHosCtr.getParent();
//	var voTempCon1 = voCon1 ;
//	if( voTempCon1.getLayout() instanceof cpr.controls.layouts.VerticalLayout){
//		//사이즈를 줄인다. 
//	}else{
//		voTempCon1 = voCon1.getParent() ;
//	}
//	
//	if( voAppContainer != voTempCon1 && voTempCon1.getLayout() instanceof cpr.controls.layouts.VerticalLayout){
//		//사이즈를 줄인다. 
//	} 
