/************************************************
 * new_swipe.module.js
 * Created at 2024. 5. 8. 오전 9:58:17.
 *
 * @author HANS
 ************************************************/
/**
...
function SlideView(container) {
	...
	this._cloneLayout = _.clone(this._layout);
...
/**
 * 반응형시, 슬라이드를 적용해야하는 스크린 명, 복수개의 경우','콤마로 구분하여 사용
 * null시 모든 스크린에 대하여 슬라이드 적용
 */
SlideView.prototype.acceptScreen = null;
...
SlideView.prototype._onScreenChange = function(e){
	/** @type String */
	var acceptScreen = this.acceptScreen;
	var vsScreenName = e.screen.name;
	if(acceptScreen != null) {
		console.log(acceptScreen.indexOf(vsScreenName));
		if(acceptScreen.indexOf(vsScreenName) != -1){
 ...
 *
 * @param {cpr.controls.Container} container
 * @param {SlideView} param
 */
exports.slidify = function(container,param) {
	var voNewSlideView = new SlideView(container);
	if(param) {
		
		Object.keys(param).forEach(function(each){
			if(each in voNewSlideView) {
				
				voNewSlideView[each] = param[each];
			}
		});
	}
	return voNewSlideView;
};
