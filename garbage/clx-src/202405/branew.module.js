/************************************************
 * branew.module.js
 * Created at 2024. 5. 8. 오후 3:51:59.
 *
 * @author HANS
 ************************************************/
...
function Swiper() {
  ...
  function calcDevice(_temp) {
  	let userAgent = _temp ===void 0? {} : _temp.userAgent;
  ...
  function calcBrowser() {
    ...
    if (isSafari()) {
      ...
      if (ua.includes('Version/')) {
        const major = ua.split('Version/')[1].split(' ')[0].split('.').map(num => Number(num));
        const minor = ua.split('Version/')[1].split(' ')[0].split('.').map(num => Number(num));
...
//          } = swiper;
          ...
          entries.forEach(_ref2 => {
          	...
          	let contentRect = _ref2.contentRect;
...
//            } = _ref2;
  ...
  function Observer(_ref) {
  	let swiper = _ref.swiper;
  	let extendParams = _ref.extendParams;
  	let on = _ref.on;
  	let emit = _ref.emit;
//    let {
//      swiper,
...
//    } = swiper;
  ...
  function updateSlidesClasses() {
    ...
    const slides = swiper.slides;
    const params = swiper.params;
    const slidesEl = swiper.slodesEl;
...
//    } = swiper;
  ...
  function transitionEmit(_ref) {
  	let swiper = _ref.swiper;
  	let runCallbacks = _ref.runCallbacks;
...
//    help
    ...
    const params = swiper.params;
    const smapGrid = swiper.snapGrid;
    const slidesGrid = swiper.slidesGrid;
    const previousIndex = swiper.previousIndex;
    const activeIndex = swiper.activeIndex;
    const rtl = swiper.trlTranslate;
    const wrapperEl = swiper.wrapperEl;
...
//    } = swiper;
  ...
  function slidePrev(speed, runCallbacks, internal) {
    ...
    const swiper = this;
    const pararms = swiper.params;
    const snapGrid = swiper.snapGrid;
    const slidesGrid = swiper.slidesGrid;
    const rtlTranslate = swiper.rtlTrnaslate;
    const enabled = swiper.enabled;
    const animating = swiper.animating;
//    const {
//      params,
...
//    } = swiper;
  ...
  function loopFix(_temp) {
  	...
  	let q = _temp ===void 0 ? {} : _temp;
  	
  	let slideRealIndex = q.slideRealIndex;
  	let slideTo =true;
  	let direction = q.direction;
  	let setTranslate = q.setTranslate;
  	let activeSlideIndex = q.activeSlideIndex;
  	let byController = q.byController;
  	let byMousewheel = q.byMousewheel;
//    let {
//      slideRealIndex,
//      slideTo = true,
//      direction,
...
//      params
//    } = swiper;
	const slideOffsetBefore = params.slideOffsetBefore;
//    const {
//      slidesOffsetBefore
//    } = params;
...
//    constructor() {
      ...
      Object.keys(tempObj).forEach(function(each){
      	swiper[each] = tempObj[each];
      });
      swiper.emit('_swiper');
      ...
      return swiper;
//    }
    this.getDirectionLabel = function(property) {
    ...
    this.getSlideIndex = function(slideEl) {
...
//      } = this;
    ...
    this.getSlideIndexByData = function(index) {
      return this.getSlideIndex(this.slides.filter(slideEl => slideEl.getAttribute('data-swiper-slide-index') * 1 === index)[0]);
    }
    this.recalcSlides= function() {
      const swiper = this;
      const slideEl = swiper.slidesEl;
      const params = swiper.params;
//      const {
//        slidesEl,
//        params
//      } = swiper;
   ...
   this.setProgress= function(progress, speed) {
      const swiper = this;
    ...
    this.emitContainerClasses= function() {
      const swiper = this;
    ...
    this.getSlideClasses= function(slideEl) {
      const swiper = this;
      if (swiper.destroyed) return '';
    ...
    this.slidesPerViewDynamic= function(view, exact) {
      if (view === void 0) {
        view = 'current';
      }
      if (exact === void 0) {
...
//      } = swiper;
    ...
    this.changeDirection= function(newDirection, needUpdate) {
    ...
    this.changeLanguageDirection= function(direction) {
      const swiper = this;
    ...
    this.destroy= function(deleteInstance, cleanStyles) {
