 ...
 * 		headerTitle		{String}  다이얼로그 헤더 타이틀
 * 		readyFunction 	{{(dialog) => void}} 다이얼로그가 열릴 때 설정할 스타일등이 있을 경우 해당 함수에서 구현하십시오.
 */
DialogKit.prototype.open = function(app, appid, width, height, handler, initValue, prop) {
	...
	app.getRootAppInstance().openDialog(appid, dialogProp, function(/* cpr.controls.Dialog */dialog) {
		...
		if(prop && prop.headerTitle) {
			
			dialog.headerTitle = prop.headerTitle;
		}else {
		...
		if(prop && prop.readyFunction) {
			prop.readyFunction(dialog);
		}
		if (handler) {
...
//2020.08.06 조한진. isPopup의 경우 앱에 isPopup를 추가시킨 것이어서 다이얼로그로 열린 화면이 일반화면으로 열릴때는 대응되지 않습니다.
/**
 * 현재 앱인스턴스가 팝업으로 열렸는지 여부를 반환합니다.
 * @param {cpr.core.AppInstance} _app
 */
DialogKit.prototype.isOpenedPop = function(_app){
	var result = false;
	return (!ValueUtil.isNull(_app.getHost()) && _app.getHost().type == "dialog") ? true : false;
}
/**
 * window open
 * @param {cpr.core.AppInstance} app 앱인스턴스
